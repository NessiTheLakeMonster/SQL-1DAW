...
SELECT ACTORES.NOMBRE, SUM(PELÍCULAS.PRESUPUESTO) AS TOTAL_PRESUPUESTO
FROM ACTORES JOIN REPARTO
ON ACTORES.CÓDIGO = REPARTO.C_ACTOR
JOIN PELÍCULAS
ON REPARTO.C_PELICULA = PELÍCULAS.CÓDIGO
...
SELECT ACTORES.NOMBRE, PELÍCULAS.CÓDIGO, PELÍCULAS.TÍTULO, PELÍCULAS.FECHA
FROM ACTORES JOIN REPARTO
ON ACTORES.CÓDIGO = REPARTO.C_ACTOR
JOIN PELÍCULAS
ON REPARTO.C_PELICULA = PELÍCULAS.CÓDIGO
    WHERE SUBSTR(TO_CHAR(PELÍCULAS.FECHA, 'YY'),1,2) BETWEEN '76' AND '95';
...
WHERE UPPER(NOMBRE) LIKE 'PACO LEÓN';

--? 12.- Actualiza año de ‘Los Mánagers’ al 1/1/2005.
UPDATE PELÍCULAS
SET FECHA = '1/1/2005'
WHERE UPPER(TÍTULO) LIKE 'LOS MÁNAGERS';
